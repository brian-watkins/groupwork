import { behavior, example, effect, fact, step, use, Context } from "best-behavior";
    ...
    .script({
      suppose: [
        fact("there is a course for another teacher", async (context) => {
          await context.withCourses(altAuthenticatedTeacher(), [
            testCourse(1),
            testCourse(2)
          ])
            .setupDB()
      ...
      observe: [
        effect("the other teacher can view the groups page", async (context) => {
          const response = await context.loadGroupsForCourse(testCourse(2))
