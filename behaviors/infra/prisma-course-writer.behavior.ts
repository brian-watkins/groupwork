...
export default behavior("PrismaCourseWriter", [
      ...
      perform: [
        step("write a new course with students", async (context) => {
          await context.writeCourse(testTeacher(1), {
            ...
            students: [testStudent(1), testStudent(2), testStudent(3)],
          })
        }),
      ],
      observe: [
        effect(
          "the course exists in the database with the correct details",
          async (context) => {
  ...
  example(testableDatabase)
    ...
    .script({
      ...
      observe: [
        effect(
          "the course exists in the database with no students",
          async (context) => {
            const courses = await context.getAllCourses(testTeacher(1))
    ...
    .script({
      perform: [
        step("write multiple courses", async (context) => {
          ...
          await context.writeCourse(testTeacher(1), {
            name: "Second Course",
            students: [testStudent(3)],
          })
        }),
      ],
      observe: [
        effect(
          "all courses exist in the database with correct details",
          async (context) => {
            await expect(
              context.getAllCourses(testTeacher(1)),
              resolvesTo(
                arrayWith(
                  [
                    objectWith({
                    ...
                    objectWith({
                      ...
                      students: arrayWith<Student>([studentName(3)]),
                    }),
                  ],
                  { withAnyOrder: true },
                ),
  ...
  example(testableDatabase)
    ...
    .script({
      ...
      perform: [
        step(
          "update the course by changing its name and modifying students",
          async (context) => {
            const courseToUpdate = await context.getCourseValue(
              testTeacher(1),
              testCourse(1),
            )
            ...
            courseToUpdate.students = courseToUpdate.students
              .filter((student) => student.name !== testStudent(3).name)
              .filter((student) => student.name !== testStudent(4).name)
      ...
      observe: [
        effect(
          "the updated course exists in the database with correct details",
          async (context) => {
            // Note that the id of the course is still saved in the test helper by the testCourse(1).name
            const updatedCourse = await context.getCourseValue(
              testTeacher(1),
              testCourse(1),
            )
    ...
    .script({
      suppose: [
        fact("there is a saved course", async (context) => {
          await context.withCourse(
            testTeacher(1),
            testCourse(4).withStudents(testStudents(4)),
          )
        }),
      ],
      perform: [
      ...
      observe: [
        effect(
          "the course name is updated but students remain unchanged",
          async (context) => {
            await expect(
              context.getAllCourses(testTeacher(1)),
              resolvesTo(
    ...
    .script({
      ...
      perform: [
        step("delete the course", async (context) => {
          const courseToDelete = await context.getCourseValue(
            testTeacher(1),
            testCourse(1),
          )
      ...
      observe: [
        effect(
          "the course and its students are removed from the database",
          async (context) => {
            const courses = await context.getAllCourses(testTeacher(1))
    ...
    .script({
      suppose: [
        fact("there are multiple courses", async (context) => {
          await context.withCourse(
            testTeacher(1),
            testCourse(1).withStudents([testStudent(1), testStudent(2)]),
          )
          await context.withCourse(
            testTeacher(1),
            testCourse(2).withStudents([testStudent(3), testStudent(4)]),
          )
        }),
      ],
      perform: [
      ...
      observe: [
        effect(
          "only the deleted course is removed, others remain intact",
          async (context) => {
            const courses = await context.getAllCourses(testTeacher(1))
  ...
  example(testableDatabase)
    ...
    .script({
      ...
      perform: [
        step(
          "another teacher attempts to save changes to that course",
          async (context) => {
            const courseToModify = await context.getCourseValue(
              testTeacher(1),
              testCourse(1),
            )
            ...
            await expect(
              context.saveCourse(testTeacher(2), courseToModify),
              rejectsWith(errorWithMessage(stringContaining("not found"))),
            )
          },
        ),
      ],
  ...
  example(testableDatabase)
    ...
    .script({
      ...
      perform: [
        step(
          "another teacher attempts to delete that course",
          async (context) => {
            const courseToDelete = await context.getCourseValue(
              testTeacher(1),
              testCourse(1),
            ...
            await expect(
              context.deleteCourse(testTeacher(2), courseToDelete),
              rejectsWith(errorWithMessage(stringContaining("not found"))),
            )
          },
        ),
      ...
      observe: [
        effect(
          "the course still exists for the original teacher",
          async (context) => {
            const courses = await context.getAllCourses(testTeacher(1))
            expect(
              courses,
  ...
  example(testableDatabase)
    .description(
      "multiple teachers can create and manage their own courses independently",
    )
    .script({
      perform: [
      ...
      observe: [
        effect(
          "each teacher's changes affect only their own courses",
          async (context) => {
            const teacher1Courses = await context.getAllCourses(testTeacher(1))
            expect(
              teacher1Courses,
  ...
  example(testableDatabase)
    ...
    .script({
      observe: [
        effect(
          "saving a course that does not exist throws an error",
          async (context) => {
            ...
            await expect(
              context.saveCourse(testTeacher(1), nonExistentCourse),
              rejectsWith(errorWithMessage(stringContaining("not found"))),
            )
          },
        ),
