...
export default behavior("PrismaTeacherAuthorization", [
  ...
  example(testableDatabase)
    ...
    .script({
      ...
      observe: [
        effect("the owner teacher is authorized to manage the course", async (context) => {
          await expect(context.canManageCourse(testTeacher(1), testCourse(1)), resolvesTo(true));
        })
      ]
    }),
  ...
  example(testableDatabase)
    ...
    .script({
      ...
      observe: [
        effect("the non-owner teacher is not authorized to manage the course", async (context) => {
          await expect(context.canManageCourse(testTeacher(2), testCourse(1)), resolvesTo(false));
        })
      ]
    }),
  ...
  example(testableDatabase)
    ...
    .script({
      observe: [
        effect("the teacher is not authorized to manage a non-existent course", async (context) => {
          await expect(context.canManageCourse(testTeacher(1), testCourse(99)), resolvesTo(false));
        })
      ]
    })
