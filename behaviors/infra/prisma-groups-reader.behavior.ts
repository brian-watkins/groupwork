...
export default behavior("PrismaGroupsReader", [
  example(testableDatabase)
    ...
    .script({
      ...
      observe: [
        effect("it gets all groups flattened from all group sets", async (context) => {
          const groups = await context.getGroupsForCourse(testCourse(1))

          expect(groups.length, is(equalTo(3)))
          expect(groups, is(arrayWith<Group>([
            groupWithMembers([
              studentName(1),
              studentName(2)
            ]),
  ...
  example(testableDatabase)
    ...
    .script({
      ...
      observe: [
        effect("it gets only the groups for the specified course", async (context) => {
          const course1Groups = await context.getGroupsForCourse(testCourse(1))
          expect(course1Groups.length, is(equalTo(1)))
          expect(course1Groups[0], is(groupWithMembers([
            studentName(1),
            studentName(2)
          ])))

          const course2Groups = await context.getGroupsForCourse(testCourse(2))
          expect(course2Groups.length, is(equalTo(1)))
          expect(course2Groups[0], is(groupWithMembers([
            studentName(3),
            studentName(4)
          ])))
        })
