...
export default behavior("PrismaCourseReader", [
  example(testableDatabase)
    ...
    .script({
      ...
      observe: [
        effect(
          "it gets the course with the correct details",
          async (context) => {
            await expect(
              context.getCourse(testTeacher(1), testCourse(1)),
              resolvesTo(
                okResultWith(
                  objectWith({
    ...
    .script({
      suppose: [
        fact("there are multiple courses with students", async (context) => {
          ...
          await context.withCourse(
            testTeacher(1),
            testCourse(2).withStudents(testStudents(3, { startingIndex: 3 })),
          )
        }),
      ],
      observe: [
        effect(
          "it gets a list of all courses with their students",
          async (context) => {
  ...
  example(testableDatabase)
    ...
    .script({
      suppose: [
        fact(
          "there are courses created by different teachers",
          async (context) => {
            await context.withCourse(
              testTeacher(1),
              testCourse(1).withStudents(testStudents(2)),
      ...
      observe: [
        effect(
          "each teacher can only see their own courses",
          async (context) => {
        ...
        effect(
          "a teacher cannot access a course created by another teacher",
          async (context) => {
            await expect(
              context.getCourse(testTeacher(1), testCourse(2)),
              resolvesTo(errorResultWith(equalTo(CourseReaderError.NotFound))),
            )
          },
        ),
      ],
  ...
  example(testableDatabase)
    .description(
      "returns error when attempting to access a non-existent course",
    )
    .script({
      suppose: [
        fact("there is a course with students", async (context) => {
      ...
      observe: [
        effect(
          "it retuens an error when trying to access a non-existent course",
          async (context) => {
            await expect(
              context.getCourseById(testTeacher(1), "non-existent-id"),
              resolvesTo(errorResultWith(equalTo(CourseReaderError.NotFound))),
            )
          },
        ),
