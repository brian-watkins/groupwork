...
import { errorResultWith, groupSetWithStudents, okResultWith, setWithSize, students } from "./helpers/matchers"
    ...
    .script({
      suppose: [
        fact("there is a course for a teacher", (context) => {
          context.withCourse(testTeacher(1), testCourse(1).withStudents(testStudents(4)))
        })
      ],
      perform: [
        step("an unauthorized teacher tries to assign groups", async (context) => {
          await context.chooseGroupsOf(testTeacher(2), 2)
        })
      ],
      observe: [
        effect("the result is an error", context => {
          expect(context.getCurrentGroups(), is(errorResultWith(equalTo(AssignGroupsError.Unauthorized))))
        })
      ]
    }),
    ...
    .script({
      suppose: [
        fact("there are five members of a class that have not been in any groups", (context) => {
          context.withCourse(testTeacher(1), testCourse(1).withStudents(testStudents(5)))
        })
      ],
      perform: [
        step("choose groups of 2", async (context) => {
      ...
      observe: [
        ...
        effect("the two groups are distinct", async (context) => {
          expect(context.getCurrentGroups(), is(okResultWith(groupSetWithStudents(students(5)))))
        })
      ]
    }),
    ...
    .script({
      suppose: [
        fact("there are 11 members of a class that have not been in any groups", (context) => {
          context.withCourse(testTeacher(1), testCourse(1).withStudents(testStudents(11)))
        })
      ],
      perform: [
      ...
      observe: [
        ...
        effect("all the students are included once", async (context) => {
          expect(context.getCurrentGroups(), is(okResultWith(groupSetWithStudents(students(11)))))
        })
      ]
    }),
