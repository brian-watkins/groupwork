...
export default behavior("delete group set", [
  example(testableGroupWorkDomain)
    ...
    .script({
      ...
      observe: [
        effect(
          "attempt to delete the group set by another teacher results in an error",
          async (context) => {
            const result = await context.deleteGroupSet(
              testTeacher(2),
              testGroupSet(1),
            )
            expect(
              result,
              is(errorResultWith(equalTo(DeleteGroupSetError.Unauthorized))),
            )
          },
        ),
      ],
  ...
  example(testableGroupWorkDomain)
    ...
    .script({
      ...
      observe: [
        effect(
          "attempt to delete the group set is successful",
          async (context) => {
            const result = await context.deleteGroupSet(
              testTeacher(1),
              testGroupSet(1),
