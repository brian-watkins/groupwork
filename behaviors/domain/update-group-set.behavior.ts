...
export default behavior("update group set", [
  ...
  example(testableGroupWorkDomain)
    ...
    .script({
      ...
      observe: [
        effect("attempt to update the group set by another teacher results in an error", async (context) => {
          const result = await context.updateGroupSet(testTeacher(2), testGroupSet(1).withCourse(testCourse(1)))
          expect(result, is(errorResultWith(equalTo(UpdateGroupSetError.Unauthorized))))
        })
      ]
    }),
  ...
  example(testableGroupWorkDomain)
    ...
    .script({
      ...
      observe: [
        effect("attempt to update the group set is successful", async (context) => {
          const result = await context.updateGroupSet(testTeacher(1), testGroupSet(1).withCourse(testCourse(1)))
          expect(result, is(okResultWith(objectWithProperty("id", equalTo(testGroupSet(1).id)))))
        })
      ]
    })
])