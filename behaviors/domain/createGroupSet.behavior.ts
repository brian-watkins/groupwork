...
export default behavior("creating group sets", [
      ...
      observe: [
        effect("the group set is created successfully", async (context) => {
          ...
          expect(
            ...
            is(
              okResultWith(
                objectWithProperty("name", equalTo("Project Teams")),
              ),
            ),
          )
        }),
  ...
  example(testableGroupWorkDomain)
    .description(
      "when a teacher who does not own the course attempts to create a group set",
    )
    .script({
      suppose: [
      ...
      observe: [
        effect("an Unauthorized error is returned", async (context) => {
          ...
          expect(
            result,
            is(errorResultWith(equalTo(GroupSetError.Unauthorized))),
          )
        }),
      ],
    }),
